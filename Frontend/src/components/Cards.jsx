...
const Cards = ({ item, onAddToCart, cardClassName = "w-full" }) => {
  return (
    <div className={`${cardClassName} hover:scale-105 duration-300`}>
      <div className="card w-full shadow-sm group mt-4 sm:mt-6 md:mt-8 relative overflow-hidden rounded-lg bg-white dark:bg-slate-800 border dark:border-slate-700">
      <figure className="overflow-hidden">
          <img
            src={item.image}
            alt={item.name}
            className="w-full h-48 sm:h-56 md:h-64 lg:h-72 object-cover group-hover:scale-105 duration-300 transition-all ease-in-out"
          />
        </figure>
        <div className="p-4">
